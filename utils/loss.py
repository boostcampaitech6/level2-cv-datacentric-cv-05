...
def get_geo_loss(gt_geo, pred_geo):
    ...
    d1_pred, d2_pred, d3_pred, d4_pred, angle_pred = torch.split(pred_geo.type(torch.float32), 1, 1)
...
class EASTLoss(nn.Module):
    ...
    def forward(self, gt_score, pred_score, gt_geo, pred_geo, roi_mask):
        ...
        iou_loss = torch.sum(iou_loss_map * gt_score) / (torch.sum(gt_score) + 1e-6)
